...
- **Feature/ticket identifiers**: e.g. ticket IDs (e.g. `TICKET-123`), branch names, or short feature labels. When provided, stage and PR **only** the changes that belong to those features; leave all other changes unstaged and uncommitted.
...
- Write the commit message **in English** (per `openspec/specs/base-standards.mdc`).
- Use **Conventional Commits** format (per `openspec/specs/backend-standards.mdc` and `openspec/specs/frontend-standards.mdc`):
  - **Subject line**: `<type>(<scope>): <short imperative description>`
    - Types: `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `style`
    - Scope: affected area in lowercase (e.g. `contacts`, `auth`, `ui`, `dynamodb`)
    - Examples: `feat(contacts): add POST /contacts handler`, `fix(auth): handle expired token error`
  - **Body** (if needed): bullet points describing what changed and why. Reference ticket IDs here if they apply (e.g. `Closes TICKET-123`).
- Do not commit secrets, `.env`, `.env.local`, or build artifacts (`dist/`, `.next/`, `node_modules/`).
...
- Create or update the PR for the current branch:
  - **Title**: `[TICKET-ID] <short description>` (e.g. `[TICKET-123] Add POST /contacts endpoint`)
...
- `openspec/specs/base-standards.mdc` — English-only for all technical artifacts
- `openspec/specs/backend-standards.mdc` — Git workflow and Conventional Commits format (backend)
- `openspec/specs/frontend-standards.mdc` — Git workflow and Conventional Commits format (frontend)
- `openspec/.commands/develop-backend.md` and `openspec/.commands/develop-frontend.md` — PR creation with `gh`
